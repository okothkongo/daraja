defmodule Daraja.HTTP.RetryTest do
  ...
  test "retryable?/1 returns false for non-http errors" do
    refute Retry.retryable?({:error, :auth_failed, %{}})
    refute Retry.retryable?({:error, :timeout})
    refute Retry.retryable?({:ok, :ignored})
  end

  test "does not retry two-tuple errors even when enabled" do
    Application.put_env(:daraja, :http_retry, enabled: true, max_attempts: 3, base_ms: 1)
